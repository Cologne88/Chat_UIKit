...
public class TUIGroupChatViewController: TUIBaseChatViewController, V2TIMGroupListener {
    ...
    func jump2GroupPinHighlightLine(_ originMessage: V2TIMMessage) {
        guard let msgVC = messageController as? TUIMessageController,
              let msgID = originMessage.msgID else { return }
        msgVC.findMessages([msgID], callback: { success, _, messages in
    ...
    @objc func openPendency(_ sender: Any) {
        ...
        vc.cellClickBlock = { [weak self] cell in
            guard let self = self, let pendencyData = cell.pendencyData else { return }
            if pendencyData.isRejected || pendencyData.isAccepted {
                return
            }
            guard let fromUser = pendencyData.fromUser else { return }
            V2TIMManager.sharedInstance().getUsersInfo([fromUser], succ: { [weak self] profiles in
    ...
    override func onLongSelectMessageAvatar(_ controller: TUIBaseMessageController, cell: TUIMessageCell) {
