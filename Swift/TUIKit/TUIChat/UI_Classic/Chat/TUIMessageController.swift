...
class TUIMessageController: TUIBaseMessageController, TUIChatSmallTongueViewDelegate {
    var bottomIndicatorView: UIActivityIndicatorView?
    var locateGroupMessageSeq: UInt64 = 0
    var tongueView: TUIChatSmallTongueView?
    var receiveMsgs: [TUIMessageCellData]?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        TUIChatSmallTongueManager.hideTongue(false)
    }

    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        TUIChatSmallTongueManager.hideTongue(true)
    }
    ...
    override func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView.contentOffset.y <= CGFloat(TMessageController_Header_Height) &&
            (scrollView.isTracking || scrollView.isDecelerating) &&
            !(messageSearchDataProvider?.isOlderNoMoreMsg ?? false) &&
            !(indicatorView?.isAnimating ?? false)
        {
            indicatorView?.startAnimating()
        } else if isScrollToBottomIndicatorViewY(scrollView) {
            if (scrollView.isTracking || scrollView.isDecelerating) &&
                !(messageSearchDataProvider?.isNewerNoMoreMsg ?? false) &&
                !(bottomIndicatorView?.isAnimating ?? false)
            {
                bottomIndicatorView?.startAnimating()
            }
            if isInVC {
                TUIChatSmallTongueManager.removeTongue(type: .scrollToBoom)
                TUIChatSmallTongueManager.removeTongue(type: .receiveNewMsg)
            }
        } else if isInVC && receiveMsgs?.count == 0 &&
            tableView.contentSize.height - tableView.contentOffset.y >= TUISwift.screen_Height() * 2.0
        {
    ...
    override func showReplyMessage<T: TUIBubbleMessageCell>(_ cell: T) {
        var originMsgID = ""
        var msgAbstract = ""
    ...
    func onChatSmallTongueClick(_ tongue: TUIChatSmallTongue) {
        switch tongue.type {
        case .scrollToBoom:
