...
public class TUITranslationExtensionObserver: NSObject, TUIExtensionProtocol {
    ...
    public func onGetExtension(_ extensionID: String, param: [AnyHashable: Any]?) -> [TUIExtensionInfo]? {
            ...
            extensionID == TUICore_TUIChatExtension_PopMenuActionItem_MinimalistExtensionID
        {
            ...
            if extensionID == TUICore_TUIChatExtension_PopMenuActionItem_ClassicExtensionID {
                ...
                info.icon = UIImage(named: TUISwift.tuiChatImagePath_Minimalist("icon_translate"))
                info.weight = 800
            }
            info.onClicked = { _ in
    ...
    func isSelectAllContentOfMessage(_ cell: TUIMessageCell) -> Bool {
        if let textCell = cell as? TUITextMessageCell {
            if textCell.selectContent.isNilOrEmpty {
                return true
            } else {
            ...
            if refCell.selectContent.isNilOrEmpty {
                return true
            } else {
            ...
            if replyCell.selectContent.isNilOrEmpty {
                return true
            } else {
